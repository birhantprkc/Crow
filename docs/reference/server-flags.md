...
| `--mmproj` | `models\qwen-next-gguf\mmproj-F16.gguf` | the vision projector (#170). The same repository ships it in its ROOT, so a download filtered to the quant folder misses it. **Loading is what the sources support, not answering:** llama.cpp issue #27886 is open with three reports of missing image content. Cost on this line is unmeasured — the model sits at the card edge under `-ncmoe 30`: measured 2026-09-01, the operating point takes **30,984 MiB of 32,607** and leaves **1,059 MiB**, with no load peak above that (3 MiB over 57 samples). The projector is loaded by this line; whether an image PREFILL fits in the remaining gigabyte is the open half — Qwen-VL asks for at least 1,024 image tokens |
