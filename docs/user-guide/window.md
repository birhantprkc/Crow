...
| Delegation | a subtask is a card in the pinned **Subtasks** panel beside the goal and git cards — spot, state, token count; running ones on top, finished ones in a folded group — never a block of the scrolling chat (#255) — and a child row under its root chat in the rail, marked `⑂`. Clicking either jumps to the card; a subtask is never opened as a chat. Cards keep breathing outside a turn, and Stop cancels the subtasks with the turn |
