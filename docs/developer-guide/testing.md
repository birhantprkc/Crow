...
`tools/test_pathtracer_kit.py` (15 cases) is the kit's suite: the checker red per broken link, and
...
triangle, the emission cap, the scene rules, despeckle. The node half is skipped without `node`.
