// Scaffold: a small cut-away room, path traced. Copy this folder, then replace
// the content section with your own scene. Keep the rules in the kit's SKILL.md:
// room >= 64 voxels wide, every prop >= 8-16 voxels per dimension, one palette
// colour per material (never vertexColors), lamps through addLamp, emission <= 1.
...
// ---- props (each >= 8 voxels per dimension) --------------------------------
// cabinet against the left wall, two drawers with knobs
...
// round table with a bowl of fruit
...
// potted plant with a glass vase beside it
g.cyl(8, 50, 3, 2, 8, P.brown); g.blob(8, 14, 50, 5, 6, 5, (x, y, z) => (hash(x, y, z) < 0.5 ? P.green : P.green2));
g.cyl(40, 4, 1.6, 22, 26, P.glass, 'glass');
// floor lamp: brass base and pole; addLamp adds the solid glowing shade + light
const LX = 54, LZ = 48;
g.cyl(LX, LZ, 3, 2, 3, P.brass, 'metal'); g.box(LX, 4, LZ, LX, 33, LZ, P.brass, 'metal');
